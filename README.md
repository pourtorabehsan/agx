...
Run sandboxed Claude Code or Codex sessions in Docker.

`agx` is a small Go CLI that launches one container per session. Each session gets its own workspace directory; credentials, the knowledge base, and agent config are persisted on the host and bind-mounted in.

## Why

- **Isolation.** The agent bypasses its own approval prompts inside a container instead of on your host.
- **Reproducibility.** A single image (`agx:latest`) pins the toolchain — Node, Go, `gh`, `rg`, `claude`, `codex` — so every session starts from the same place.
- **Persistence where it matters.** SSH keys, `gh` auth, agent credentials, and a shared knowledge base survive across sessions; the rest of the container is thrown away.
...
agx run --agent claude "..."                        # run with Claude instead of Codex
agx run --model <model-id> "..."                    # model override for the selected agent
...
agx upgrade                                         # reinstall agent skills/instructions from the current image
...
Headless runs tee stdout/stderr into `<workspace>/.agx/session.log`. Interactive runs attach a TTY and skip the log file. Detached runs (`-d` / `--detach`) run the container in the background, printing the workspace name and log path to stderr on start, and are mutually exclusive with `--interactive`. Both `run` and `resume` support `-d`, `-i`, `--agent`, `--model`, `--memory`, and `--cpus`.

Every non-empty headless or interactive prompt automatically prepends `invoke conduct skill`, so the `conduct` skill is the default entry point for all task-oriented runs. Pass `--no-conduct` to skip this (useful for simple or exploratory prompts). Pass `-i` without a prompt to drop into a plain agent session. `--agent` defaults to `codex`; `resume` and `attach` use the workspace's recorded agent unless overridden.
...
If `--name` is omitted, the workspace name is a Docker-style 12-character random hex ID. Use `agx ps` for the created time, agent, model, and prompt. If `--name` is provided, it is slugified and used exactly.
...
│   └── <name>/
│       └── .agx/
│           ├── meta.json         prompt, repos, start time, agent, model
│           ├── prompt.txt
│           ├── session.log
...
image/           Docker image
  Dockerfile     ubuntu:24.04 + node + gh + claude + codex
  entrypoint.sh  picks headless vs interactive vs resume based on AGX_MODE
  bootstrap.sh   the first-run wizard above
  plugin/        agent skills/instructions baked in
    CLAUDE.md    Claude instruction placeholder
    AGENTS.md    Codex instruction placeholder
    skills/
      capture/   SKILL.md
      conduct/   SKILL.md
      kb/        SKILL.md
